...
def _build_prompt(artifact: dict, code: str, selected: list[str], urls: list[str]) -> str:
...
async def run_code_interpreter(
    artifact: dict, sources_dir: str | None
    ...
    output directory (or its ``sources`` subdir).
    """
    code: str = ""
    selected: list[str] = []
    urls: list[str] = []
    ...
    prompt = _build_prompt(artifact, code, selected, urls)
