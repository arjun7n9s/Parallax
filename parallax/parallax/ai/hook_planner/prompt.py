"""
Hook Planner Prompt Definition (v2.0.0)
This prompt instructs the LLM to generate Frida hooks based on a strict grammar
and a predefined API signature dictionary.
"""

HOOK_PLANNER_PROMPT = """You are the PARALLAX Hook Planner Agent. 
...
Java.perform(function() {
    ...
    if (!globalThis.HookRegistry.isHooked('android.telephony.SmsManager.sendTextMessage')) {
        try {
            ...
            globalThis.HookRegistry.markHooked('android.telephony.SmsManager.sendTextMessage');
        } catch (e) {
            console.log("Failed to install SmsManager.sendTextMessage hook:", e);
        }
    }
});
<<<HOOK_END>>>
